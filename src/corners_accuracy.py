...
spain_corners = wyscout_spain_matches.query("type_primary == 'corner'").query(
    "team_name == 'Spain'"
)
...
# Pitch initialization
pitch = VerticalPitch(
    pitch_color="#2f8c58", line_color="white", pitch_type="wyscout", half=True
)
...
# drop undet
spain_corners = spain_corners.dropna(subset=["pass_accurate"])


for index, row in spain_corners.iterrows():
    color = colors[row.pass_accurate]
    label = f"{['Inaccurate', 'Accurate'][row.pass_accurate]} Corner"
...
plt.savefig("results/plots/corner_accuracy.png", bbox_inches="tight")
plt.show()
